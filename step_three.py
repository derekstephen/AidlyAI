...
# First time download wordnet + averaged perceptron tagger
...
df_imp = df[["EIN", "NAME", "F9_03_PZ_MISSION", "MISSION", "WORDS", "POS", "STEMMER", "LEMMATIZATION", "TF"]].copy()


# START STEP TWO CODE

# Create Bi-gram and tri-gram with NLTK ngrams Module
df_imp['BIGRAMS'] = df_imp['WORDS'].apply(lambda x: ngrams(x, 2))
df_imp['TRIGRAMS'] = df_imp['WORDS'].apply(lambda x: ngrams(x, 3))

# END STEP TWO CODE


# START STEP THREE CODE

# First time load Spacy English Core Web Models
nlp = spacy.load('en_core_web_sm')

df_imp['DOC'] = df_imp['F9_03_PZ_MISSION'].apply(lambda x: nlp(str(x)))
